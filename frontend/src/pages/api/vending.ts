...
export const getProductList = async (): Promise<Product[]> => {
  const res = await fetch(new URL("http://192.168.148.225:8000/api/vending/productslist/"));
...
export const putProductQuantity = (id:number,quantity:number) =>{
  fetch("http://192.168.148.225:8000/api/vending/products/" + id, {
    method: "PUT",
    headers: {
...
export const postOrderMethod = (product_id:number,order_method_id:number) =>{
  fetch("http://192.168.148.225:8000/api/vending/orders/", {
    method: "POST",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
